...
function showData(use) {
  ...
  dataArray.forEach(function (r) {
    ...
    if (r.TrangThaiXuong == '03 Đang Tiếp Nhận') {
      div = "Đang TN"; row.setAttribute('class', 'ChoSuaChua');
...
function getNexttime(KhoangSC) {
  ...
  for (var a in dataArray1) {
    ...
    if (r.TrangThaiSCC !== "Dừng SC") {
      if ((new Date(DoiNgayDangKy(r.TimeEndGJ))).valueOf() > time.valueOf()) {
        time = new Date(DoiNgayDangKy(r.TimeEndGJ));
      }
    }
  }
  nexttime = time;
  ...
  for (var a in ojb) {
    ...
    if (ojb[a].BienSoXe == BienSoXe.value) {
      ...
      if (ojb[a].TrangThaiSCC == "Đang SC") {
        $("#buttonSCC").html('<button  type="button" class="btn btn-danger me-2" onclick="DungCongViecGJ()">Dừng CV!</button>&nbsp &nbsp <button  type="button" class="btn btn-primary me-2" onclick="KetThuc()">Kết Thúc!</button>')
        return
      } else { $("#buttonSCC").html('<button  type="button" class="btn btn-success me-2" onclick="BatDauSC()">Bắt Đầu!</button>&nbsp &nbsp <button  type="button" class="btn btn-primary me-2" onclick="KeHoach()">Kế Hoạch!</button>'); return }
      return
    }
  }
}
